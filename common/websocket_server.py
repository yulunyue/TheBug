...
class WebSocketHandler(StreamRequestHandler):
    ...
    def send_text(self, message):
        ...
        if isinstance(message, bytes):
            ...
            print('Can\'t send message, message has to be a string or bytes. Given type is %s' % type(
                message))
            return False
    ...
    def handshake(self):
        ...
        self.server._url = message.split('\r\n')[0].split(' ')[1]
...
class JsonWeb(WebsocketServer):
    ...
    def send(self, key, data):
        ...
        for d in self.clients:
            if d.get("name") == key:
                self.send_message(d, data)


def run_thread(port):
    rt = JsonWeb(port)
    _thread.start_new_thread(rt.serve_forever, ())
    return rt
